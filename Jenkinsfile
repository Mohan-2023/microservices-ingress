pipeline {
    ...
    environment {
        DOCKER_HUB_REPO = 'mohan598/techsolutions-app'
    ...
    stages {
        stage('Checkout') {
            steps {
                ...
                git branch: 'main', url: 'https://github.com/Mohan-2023/microservices-ingress.git'
            }
        }
        ...
        stage('Deploy to Kubernetes') {
            steps {
                ...
                script {
                    withCredentials([[$class: 'AmazonWebServicesCredentialsBinding', credentialsId: 'aws-creds']]) {
                        sh "sed -i 's|mohan598/techsolutions-app:latest|mohan598/techsolutions-app:${env.IMAGE_TAG}|g' k8s/deployment.yaml"
